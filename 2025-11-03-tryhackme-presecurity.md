# Day 1 - Notes on cybersecurity & TryHackMe PreSecurity
**Date:** 2025-11-03
**Platform:** Youtube & TryHackMe - Module:Intoduction to Cyber Security

## What I Learned
- Cybersecurity is the practice of preventing network,programs,computer system and its component from attact from hackers.
- Hackers attacks both individual and company. some of the individual attacks are man in the middle attack, malware attack, phishing attack. and some company attack include distributed
- denial of service(DDoS),DNS hijacking, and also SQL injection(SQLi)

- There are three type of hackers: Black hat hacker, Grey hat hacker, and White hat hacker(Ethical hacker).
- hacking is the exploitation of vulnerable computer network to obtaing infomation and credentials.
  
